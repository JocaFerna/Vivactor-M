...
def perform_analysis():
    ...
    print(f"Starting analysis for URL: {url_path}")
    # Convert to absolute path to avoid "current directory" confusion
    absolute_url_path = os.path.abspath(url_path)

    print(f"Absolute URL path: {absolute_url_path}")

    # Check if the URL path is a directory
    if os.path.isdir(absolute_url_path):
        ...
        if not os.path.exists(os.path.join(absolute_url_path, ".git")):
            ...
            tmp.tmp_config.set("Analysis Settings", "output_path", os.path.join(os.getcwd(), "code2DFD_output", repo_name.replace("/", "--"),"local"))
            print(f"\nStart extraction of DFD for {repo_name} at {datetime.now().strftime('%H:%M:%S')}")
            codeable_models, traceability_content = DFD_extraction()
            print(f"Finished: {datetime.now().strftime('%H:%M:%S')}")

        else:
            print(f"The provided local path '{url_path}' is a git repository, using it directly.")
            repository = Repository(path_to_repo=url_path)
        ...
        print(f"The provided URL path '{url_path}' is not a local directory, treating it as a remote repository URL and cloning it.")
